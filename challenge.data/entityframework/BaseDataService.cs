...
{
    internal class BaseDataService<T> : IDataService<T> where T : IdentityBase, new()
    {
        protected ChallengeDbContext _db;

        public BaseDataService()
        {
            _db = new ChallengeDbContext();
        }
        public List<ValidationResult> ValidateModel(T model)
        {
            ValidationContext v = new(model);
